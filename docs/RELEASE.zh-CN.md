...
手动触发的 `release-candidate.yml` 是唯一自动创建 Release 的入口。它要求输入与源码版本完全一致的 `v主版本.次版本.修订号` 标签，依次通过三平台严格签名作业，下载全部产物，并拒绝缺失、重复或未声明文件，最后只为准确的已验证提交创建 GitHub **草稿 Release**，绝不会自动公开。实际发布者必须在 GitHub 中复核发行说明、产物清单和签名结果后手动发布。
...
创建草稿候选仅在最终作业授予 `contents: write`；受保护的平台 Secret 通过同仓库可复用工作流传给签名构建。预览构建与普通 CI 仍保持只读权限。若标签或草稿已经存在，流程会失败，不覆盖既有 Release 产物。
